...
def get_grad_norm(model, norm_type=2):
    ...
    """
    parameters = filter(lambda p: p.grad is not None, model.parameters())
    parameters = torch.cat([p.grad.data.flatten() for p in parameters])
    return parameters.norm(norm_type)
