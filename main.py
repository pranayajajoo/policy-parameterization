...
from env.strategies.DMControlSuiteStrategy import DMControlSuiteEnvFactory


# Global variables
DEFAULT_MAX_NUM_PROCESSES = 1
DEFAULT_NUM_RUNS = 1


@click.command(help="Given an environment, agent name, and an integer" +
               "index, trains the agent on the environment for the " +
               "hyperparameter setting with corresponding to the integer " +
               "index. Indices higher than the number of hyperparameter " +
               "settings will wrap around and perform subsequent runs. For " +
               "example, if there are 10 hyperparameter settings and the " +
               "integer index is 11, then this " +
               "will correspond to the 2nd run of the first hyperparameter " +
               "setting, with a different random seed from the first run")
...
@click.option("--rerun", is_flag=True, help="If we're reruning " +
              "the experiment for the best hyperparameters, then we need " +
              "use a different starting random seed", type=bool, default=False,
              required=False)
def run(env_json, agent_json, index, monitor, after, save_dir, rerun):
    """
...
def main(agent_config, env_config, index, monitor, after,
    ...
    save_dir = os.path.join(save_dir, env_config["env_name"] + "_" +
                            agent_config["agent_name"] + "_" +
                            agent_config["parameters"]["policy_type"][0] + "/")
    # Run the experiments
    ...
    # Choose the platform-specific strategy (where "strategy" refers to the
    # eponymous software design pattern) that the environment will implement
    ...
    eval_env = environment.Environment(env_config, RANDOM_SEED, strategy, factory)
    ...
    # Store information needed to save intermediate results
    exp.save_cache = {
        "run_data": run_data,
        "data": data,
        "hp_sweep": hp_sweep,
