...
def sweeps(parameters, index):
    ...
    for key in parameters:
        ...
        if accum == 0 or num == 0:
            print(f"accum: {accum}, num: {num}")
            print(parameters)
            print(key)
            raise ValueError("hyperparameter sweep must have at least one " +
                             "configuration")
...
def hold_constraint(data, constraint_hypers):
    """
    Returns the hyperparameter settings indices and hyperparameter values
    of the hyperparameter settings satisfying the constraints constant_hypers.
    ...
    for index in data["experiment_data"]:
        ...
        # Check to see if the agent hyperparameter satisfies the constraints
        for hyper in constraint_hypers:
            constraint_vals = constraint_hypers[hyper]
            assert isinstance(constraint_vals, list)
            ...
            if agent_val not in constraint_vals:
                # Hyperparameter does not satisfy the constraints
                constraint_satisfied = False
                break
...
def _combine_two(data1, data2, config):
    ...
    # Renumber if the configuration file does not match that with which the
    # experiment was run
...
def renumber(data, hypers):
    ...
    # 2024.01.24: added the exception for clip_stddev
    for key in data["experiment"]["agent"]["parameters"]:
        if key not in hypers and key != "batch/replay" and key != "clip_stddev":
...
def best(data, perf=TRAIN, scale_by_episode_length=False, to=-1,
         by=np.argmax, tune_on_last_x_percent=1.0):
    """
    ...
    np.array[int], np.float32
        ...
        maximum return
    """
    if len(list(data["experiment_data"].keys())) == 0:
        raise ValueError
    elif len(list(data["experiment_data"].keys())) == 1:
        return list(data["experiment_data"].keys())[0], None

    if tune_on_last_x_percent < 1.0:
        print(f"tuning on last {tune_on_last_x_percent} percent...")
    ...
    for hyper in data["experiment_data"]:
        ...
        for run in data["experiment_data"][hyper]["runs"]:
            ...
            if tune_on_last_x_percent == 1.0:
                ...
                start_index = int(len(returns) * (1 - tune_on_last_x_percent))
                hyper_data.append(returns[start_index:].mean())

        hyper_data = np.array(hyper_data)
        hypers[hyper] = hyper_data.mean()

    ind = by(hypers)
    return ind, hypers[ind]
