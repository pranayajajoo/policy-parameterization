...
def expand_episodes(data, ind, type_='train', skip=0):
    """
    ...
    skip : int
        After episodes have been expanded, skip this many points between
        consecutive points. E.g., if the expanded episodic returns are stored
        in `r`, then this function returns `r[::skip+1]`.
    """
    data = deepcopy(data)
    ...
    if type_ == "train":
        for i in range(len(runs)):
            ...
            data["experiment_data"][ind]["runs"][i][
                "train_episode_rewards"] = run_return[::skip+1]
...
def keep_every_episode(data, i, average=False):
    data = deepcopy(data)

    for hyper in data["experiment_data"]:
        for run in range(len(data["experiment_data"][hyper]["runs"])):
            if not average:
                data["experiment_data"][hyper][
                    "runs"][run]["train_episode_rewards"] = \
                    data["experiment_data"][hyper][
                        "runs"][run]["train_episode_rewards"][::i]

                data["experiment_data"][hyper][
                    "runs"][run]["train_episode_steps"] = \
                    data["experiment_data"][hyper][
                        "runs"][run]["train_episode_steps"][::i]

                data["experiment_data"][hyper][
                    "runs"][run]["eval_episode_rewards"] = \
                    data["experiment_data"][hyper][
                        "runs"][run]["eval_episode_rewards"][::i]

                data["experiment_data"][hyper][
                    "runs"][run]["eval_episode_steps"] = \
                    data["experiment_data"][hyper][
                        "runs"][run]["eval_episode_steps"][::i]
...
def combine_all(data):
    ...
    keys = list(data["experiment_data"].keys())
    for k in keys:
        del new_data["experiment_data"][k]

    new_data["experiment_data"][0] = {}
    new_data["experiment_data"][0]["runs"] = []
    new_data["experiment_data"][0]["agent_hyperparams"] = None
    for k in keys:
        new_data["experiment_data"][0]["runs"].extend(
            data["experiment_data"][k]["runs"]
        )
